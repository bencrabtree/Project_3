...
def get_user_tweets(username): #input is user we want to get twitter data on
	if username in CACHE_DICTION:
		return CACHE_DICTION[username]
	else:
		twitter_results = api.user_timeline(username) #get from internet
		CACHE_DICTION[username] = twitter_results #adding key and value to dictionary
...
#unique takes in two paramters:
# x is a tuple being proposed to be added to list of tuples y
# if the user_id of x does not already exist in y, return true
...
curs.execute('CREATE TABLE Users (user_id TEXT PRIMARY KEY, screen_name TEXT, num_favs NUMBER, description TEXT)')
user_list = []
mentioned_list = []
for tweet in umich_tweets:
	user_tup = tweet['user']['id_str'], tweet['user']['screen_name'], tweet['user']['favourites_count'], tweet['user']['description']
	if (unique(user_tup, user_list)): #if user isnt already in table
		user_list.append(user_tup)

	if tweet['entities']['user_mentions'] != []: #if a user was mentioned
		for element in tweet['entities']['user_mentions']: #pull out their screen_name
			mentioned_list.append(element['screen_name']) #store in a list

for x in mentioned_list:
	user_tweets = get_user_tweets(x) #get data for each mentioned user
	for tweet in user_tweets:
		user_tup = tweet['user']['id_str'], tweet['user']['screen_name'], tweet['user']['favourites_count'], tweet['user']['description']
		if (unique(user_tup, user_list)): #if user isnt already in list
			user_list.append(user_tup) #add user to list

curs.execute('DROP TABLE IF EXISTS Tweets') #creating tweets table
curs.execute('CREATE TABLE Tweets (tweet_id TEXT PRIMARY KEY, text_ TEXT, user_posted TEXT, time_posted DATETIME, retweets NUMBER)')
tweet_list = []
for tweet in umich_tweets:
	tweet_tup = tweet['id_str'], tweet['text'], tweet['user']['id_str'], tweet['created_at'], tweet['retweet_count']
	if (unique(tweet_tup, tweet_list)): #if tweet isnt already in list
		tweet_list.append(tweet_tup)
conn.commit()

for x in user_list: #all unique users
	curs.execute('INSERT INTO Users VALUES (?,?,?,?)', x)
conn.commit()

for x in tweet_list: #all unique tweets
	curs.execute('INSERT INTO Tweets VALUES (?,?,?,?,?)', x)
conn.commit()
...
screen_names = []
curs.execute('SELECT screen_name FROM Users')
for x in curs:
    screen_names.append(x[0]) #selecting single column returns a one value tuple

# Make a query to select all of the tweets (full rows of tweet information)
# that have been retweeted more than 10 times. Save the result
# (a list of tuples, or an empty list) in a variable called retweets.
retweets = []
curs.execute('SELECT * FROM Tweets WHERE retweets > 10')
for x in curs:
    retweets.append(x)
...
favorites = []
curs.execute('SELECT description FROM Users WHERE num_favs > 500')
for x in curs:
    favorites.append(x[0]) #selecting single column returns a one value tuple

# Make a query using an INNER JOIN to get a list of tuples with 2
# elements in each tuple: the user screenname and the text of the
# tweet. Save the resulting list of tuples in a variable called joined_data2.
joined_data = []
curs.execute('SELECT screen_name, text_ FROM Users INNER JOIN Tweets ON Tweets.user_posted = Users.user_id')
for x in curs:
...
joined_data2 = []
curs.execute('SELECT screen_name, text_ FROM Users INNER JOIN Tweets ON Tweets.user_posted = Users.user_id ORDER BY Tweets.retweets')
for x in curs:
